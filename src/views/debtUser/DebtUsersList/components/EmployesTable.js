...
import { getDebtUsers, setTableData } from '../store/dataSlice'
...
const CompanyTable = () => {
    ...
    const { pageIndex, pageSize, search, total } = useSelector(
        (state) => state.debtStore.data.tableData
    )

    const filterData = useSelector((state) => state.debtStore.data.filterData)

    const loading = useSelector((state) => state.debtStore.data.loading)

    const data = useSelector((state) => state.debtStore.data.debtUsersList)
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                cell: (props) => {
                    const row = props.row.original
                    return <span>{row.address
                    }</span>
                },
            },
            {
                header: 'Доверенный пользователь',
            ...
            },
            {
                ...
                cell: (props) => {
                    ...
                    return (
                        <span className="capitalize">
                            {Intl.NumberFormat().format(row.balance)}
                        </span>
                    )
                },
            },
